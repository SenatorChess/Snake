...
public class Board extends JPanel implements ActionListener {
	...
	private Image bodyBlue;

	private enum COLOUR_STATE {
		UNCOLORED, GREEN, YELLOW, BLUE
	}
	private COLOUR_STATE colourState = COLOUR_STATE.UNCOLORED;
		...
		if (inGame) {
			...
			for (int z = 0; z < dots; z++) {
				if (z == 0) {
					...
					if (colourState == COLOUR_STATE.UNCOLORED || colourState == COLOUR_STATE.BLUE) {
						g.drawImage(bodygreen, x[z], y[z], this);
						colourState = COLOUR_STATE.GREEN;
					} else if (colourState == COLOUR_STATE.GREEN) {
						g.drawImage(bodyYellow, x[z], y[z], this);
						colourState = COLOUR_STATE.YELLOW;
					} else if (colourState == COLOUR_STATE.YELLOW) {
						g.drawImage(bodyBlue, x[z], y[z], this);
						colourState = COLOUR_STATE.BLUE;
					}
				}
			}
